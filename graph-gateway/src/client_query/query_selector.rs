...
#[derive(Debug, Clone)]
pub struct QuerySelector<T>(pub T);

#[async_trait]
impl<S, T> FromRequestParts<S> for QuerySelector<T>
where
    T: serde::de::DeserializeOwned + std::fmt::Display + Send,
    S: Send + Sync,
{
    type Rejection = QuerySelectorRejection;

    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        let Path(selector) = Path::<T>::from_request_parts(parts, state)
            .await
...
mod tests {
    ...
    fn test_router() -> Router {
        async fn handle_deployment_id_query(selector: QuerySelector<DeploymentId>) -> String {
            format!("{}", selector.0)
        }

        async fn handle_subgraph_id_query(selector: QuerySelector<SubgraphId>) -> String {
            format!("{}", selector.0)
        }
        ...
        Router::new()
            .route(
                ...
                axum::routing::post(handle_deployment_id_query),
            )
            .route(
                "/subgraphs/id/:subgraph_id",
                axum::routing::post(handle_subgraph_id_query),
            )
    }
    ...
    async fn invalid_deployment_id() {
        ...
        assert_matches!(deserialize_graphql_response_body::<()>(res.body_mut()).await, Ok(res_body) => {
            ...
            assert_eq!(res_body.errors[0].message, r#"subgraph not found: invalid id"#);
        });
    }
    ...
    async fn invalid_subgraph_id() {
        ...
        assert_matches!(deserialize_graphql_response_body::<()>(res.body_mut()).await, Ok(res_body) => {
            assert_eq!(res_body.errors.len(), 1);
            assert_eq!(res_body.errors[0].message, "subgraph not found: invalid id");
        });
    }
}
