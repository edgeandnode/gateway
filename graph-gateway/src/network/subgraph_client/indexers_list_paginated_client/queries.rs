...
use thegraph_graphql_http::{
    http::{request::IntoRequestParameters, response::Error},
    ...
    ResponseData: serde::de::DeserializeOwned,
{
    ...
    let resp: IndexerResponsePayload<ResponseData> =
    ...
    Ok(match (resp.data, resp.attestation, resp.error) {
        (Some(data), Some(_), None) => Ok(data),

        // Attestation is required for successful responses
        (Some(_), None, _) => Err(ResponseError::Failure {
            errors: vec![Error::from_static("Missing attestation")],
        }),
