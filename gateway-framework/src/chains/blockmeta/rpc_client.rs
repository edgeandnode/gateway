...
pub use self::auth::{AuthChannel, AuthInterceptor};
use self::gen::block_client::BlockClient;
pub use self::gen::BlockResp as Block;
use self::gen::{Empty, IdToNumReq, NumToIdReq};
...
/// ```
mod gen {
    include!("gen/sf.blockmeta.v2.rs");
}
...
mod auth {
    ...
    /// The `AuthChannel` is a gRPC channel with an `AuthInterceptor` middleware.
...
impl BlockmetaClient<AuthChannel> {
    /// Create a new [`BlockmetaClient`] with the given gRPC endpoint and authorization token.
    ///
    /// The client will connect to the given endpoint and authenticate requests with the given
