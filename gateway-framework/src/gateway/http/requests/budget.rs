...
use crate::{
    auth::AuthSettings,
    errors::Error,
    gateway::http::{GatewayImpl, GatewayState},
};
...
impl Budget {
    pub fn calculate<G>(state: &GatewayState<G>, settings: &AuthSettings) -> Result<Self, Error>
    where
        G: GatewayImpl,
    {
        let grt_per_usd = *state.grt_per_usd.borrow();
